...
- **完整原版命令别名与快捷命令**（ycm / 绑定玩家 / 玩家状态列表 / 日服模式 等）
- **车牌关键词外置配置**（car_keyword.json，41 car + 34 fake 关键词）

原版功能逐项对照及实现说明见 [COMPATIBILITY.md](./COMPATIBILITY.md)。
...
> 请勿同时启用其他 Tsugu/BanG Dream 插件；同名命令会被多个处理器重复响应。
...
| `max_retries` | 后端请求自动重试次数 | `3` |
| `max_gacha_draws` | 单次最大抽卡次数 | `300` |
...
| `command_aliases` | 与内置别名合并的命令别名映射（JSON） | `{}` |
...
| `查谱面 <曲目ID> [难度]` | 查询谱面，默认专家难度 | `查谱面 114 专家` |
| `分数表 [服务器]` | 查询分数表 | `分数表 jp` |
| `随机曲目 [关键词]` | 从匹配结果中随机一首曲目 | `随机曲目 lv24 ag` |
...
| `抽卡模拟 [次数] [卡池ID]` | 模拟抽卡 | `抽卡模拟 300 922` |
...
| 命令 | 说明 | 示例 |
|------|------|------|
| `ycx <档位> [活动ID] [服务器]` | 查询预测线 | `ycx 1000 177 jp` |
| `ycxall [活动ID] [服务器]` | 查询所有档位 | `ycxall 177 jp` |
| `lsycx <档位> [活动ID] [服务器]` | 查询历史档线 | `lsycx 1000 177 jp` |
...
| `绑定 <玩家ID>` | 修改游戏签名后发送玩家ID完成验证 | `绑定 10000000` |
| `解除绑定` | 解除绑定 | `解除绑定` |
...
### 原版全量对齐
- 按上游 `0b0607f31e8d93fc80646ad05537cbe43ce50913` 逐项核对全部启用命令、别名、快捷命令、用户状态和车牌流程
- 补齐 `绑定玩家`、`玩家状态列表`、`玩家默认ID`、`随机曲` 等原版命令名
- 补齐 `日服模式`、`日服玩家状态` 等服务器快捷命令
- 所有服务器参数和谱面难度支持原版模糊搜索及中文难度名称

### 可靠性修复
- 修复 `抽卡模拟 300 922` 被误判为卡池ID且最多只能抽100次
- 修复模糊搜索响应未解包，导致所有服务器模糊匹配实际失效
- 修复绑定验证和取消消息被 @唤醒规则拦截、解绑时可误填其他玩家ID
- 修复空车列表仍调用拒绝空数组的后端接口
- 修复用户数据异常、默认玩家下标越界、业务失败响应可能引发二次异常
- 车牌提交补充头像，优先使用适配器头像并为 QQ 提供回退地址
- 新增请求超时、重试次数和单次最大抽卡数配置
